...
class YOLOPAFPN(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        #self.backbone = CSPDarknet(depth, width, depthwise=depthwise, act=act)
        self.backbone = CSPDarknet(depth, width, out_features=in_features, depthwise=depthwise, act=act)  # clw modify
        self.in_features = in_features
        self.in_channels = in_channels
        Conv = DWConv if depthwise else BaseConv
        ...
        # )
        self.lateral_conv0 = BaseConv(
            int(in_channels[3] * width), int(in_channels[2] * width), 1, 1, act=act  # clw modify
        )
        self.C3_p4 = CSPLayer(
    ...
    def forward(self, input):
        ...
        ###### clw modify
        out_features = self.backbone(input)  # 最开始会进行依次get_model_info，输入一个宽高为64x64的假图，out_features包含了4个tensor，
                                             # 维度分别为 dark2: (1, 64, 16, 16), dark3: (1, 128, 8, 8), ....
        features = [out_features[f] for f in self.in_features]
